...
Tests: 41 new tests across `core/src/core.test.ts` (28 — `if_exists` × 4 modes + batch + strict-schema-projected-shape, `has_broken_links`/`include_broken_links` × single/list/no-table-yet/backfill-on-later-create, `summary`), `core/src/seed-packs.test.ts` (1 — the new Getting Started subsection), `src/vault.test.ts` (12 — REST parity for `if_exists`/`summary`/`has_broken_links`/`include_broken_links`). Gates: `bun run typecheck` clean; `bun test ./src/` 2139 pass / 1 skip / 0 fail; `bun test ./core/src/` 1040 pass / 0 fail. Known flake: mirror-routes "two PUTs" #558 (unrelated, not encountered this run).
