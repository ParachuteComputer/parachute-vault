...
- Schema 32 adds per-note history compaction scheduling hints: bounded indexed selection replaces repeated whole-history aggregation, no-op attempts wait for another history write, and zero-live-byte notes are excluded. Deleted-note history remains eligible; doctor reports bounded hint-drift warnings without repair.
...
- **Link edges with an invisible endpoint are suppressed on every graph surface (#718, refs #714).** rc.3 shipped with #714 named as a known follow-up: resolved link *edges* were still computed scope-blind, so a `work`-scoped reader whose only outbound link resolved to a personal note saw `link_count` 1, matched `has_links=true`, and was told its own bracket was NOT broken — three answers about a note it owns, every one of them determined by a note it can never see, and every one of them moving when that hidden note is deleted. That direction is now closed. `getLinkCounts` takes an optional visibility predicate and, under scope, counts only visible neighbours in both directions (a visible self-loop stays degree 2); with no predicate the original grouped SQL and results are unchanged, so **unscoped readers are byte-identical** — no extra query, no extra content fetch. `sqlHasLinks` lifts **both** polarities out of SQL under scope — `true` is not a superset either — and `narrowByVisibleLinks` re-decides per note at six narrowing points across MCP list/aggregate and REST list/aggregate/FTS/semantic. A content wikilink that resolved to a hidden target is re-parsed from the visible source's own content and folded into broken links only when no visible candidate remains, so a stale hidden-target edge that still has a visible same-named candidate is suppressed without being called broken; content is fetched only for sources that actually have a hidden outbound target. `/unresolved-wikilinks` gets the same deduplicated, bounded fold-in so the third surface is stable too. Structured (non-content) links are suppressed but never named broken, because their original caller string is no longer stored. Widens #707/#712: REST FTS and semantic search were missed by both and now apply the ambiguity, brokenness and link-presence filters after tag filtering. No edge is ever synthesised, and no traversal or path algorithm changed. `order_by=link_count` deliberately remains **global** degree, so an ordering can disagree with a scoped `linkCount` — written down in `docs/HTTP_API.md`, the tag-scoped-token contract and the MCP manifest, along with the pagination/cursor consequences and the structured-link limitation. Direction 1 (an edge *appearing* for a scoped reader once a hidden competing candidate is deleted) is deferred to a second PR; **#714 stays open**.
- **A stable with no matching rc on npm is refused (#715, closes #697).** Vault's `decidePublish` had no `matchingRcVersions` gate, so a stable `X.Y.Z` merged to `main` would publish to `@latest` with no `X.Y.Z-rc.*` ever on npm — the shape that shipped 0.7.13 through 0.7.16. Ports the registry half of hub#870 / surface#215: same rule, same refusal message, and rcs of a *different* core do not satisfy it. Never-published packages still skip earlier. The git suffix-drop *diff* gate remains unported.
- **`docs/HTTP_API.md` gains a per-door support table, bun vs cloud (#720, closes #719).** Query and write params were documented with no door qualification, while on the hosted door several are silently ignored — `has_broken_links`, `include_broken_links`, `exclude_path_prefix`, `aggregate[...]`, `if_exists`, batch `summary` — repeated `?tag=a&tag=b` takes the first occurrence only, and `search_mode` / `sort` under `?search=` answer 200 plus an `unsupported_param` warning. Adds the table under `## Conventions` with every row cited to the parsing line in both repos, plus the rule that every cloud parity PR updates it. Docs only.
- **Cloud's REST aggregate support documented (#722, refs #719, cloud#286).** Cloud B2 landed the REST aggregate parser, so the aggregate row and the cloud-coverage paragraph now describe rollups on both cloud doors, the four REST exclusions, and cloud#291. Docs only.
- **Per-door rows for cloud B1 debt, B7, B8 and B9 (#732, closes #730).** The first real exercise of #720's update rule: six rows replaced and four added so the hosted door no longer reads as ignoring features cloud#290 / #298 / #300 / #302 have shipped, plus a title-fallback line and the `has_ambiguous_links` / `include_ambiguous_links` rows. Docs only.
- **Merges `main` (the 0.7.9-rc.3 promotion) into the line.** Changelog section and version bump only; `main` and `next` share a history again.

Known follow-up: **#714** stays open for the appearing-edge direction — an edge a scoped reader could not see becoming visible after an out-of-scope candidate is deleted. The disclosure direction closed here; that one needs the same link-graph contract work.

Do not suffix-drop 0.7.9 until this rc has lived.

## [0.7.9-rc.3] - 2026-09-05

**Ambiguous links become queryable, scoped link answers stop moving under out-of-scope edits, and note updates become transactional.** The eleven commits on `next` since rc.2 (#701–#717). Schema goes 27 → 28; the migration is gated and rewrites no notes.

- **`has_ambiguous_links` / `include_ambiguous_links` (#707, closes #581).** An ambiguous link target (≥2 notes matched, so no link was created) existed only in the transient `ambiguous_link` warning on the write response — nothing was persisted, so `has_broken_links: false` actively classified the note as CLEAN and a later audit could not find the collision at all. Now persisted in its own lazily-created `ambiguous_wikilinks` table mirroring `unresolved_wikilinks` (3-column PK, `ON DELETE CASCADE`) plus the candidate count. Kept separate from the broken-link table on purpose: folding would have silently widened `has_broken_links: true` for every existing caller. Filter + folding option on both doors, same shape as `has_broken_links`/`include_broken_links`; `has_ambiguous_links` is bound into the cursor query-hash. Self-healing via `refreshAmbiguousLinks` after a create, delete or repath — a row that now resolves to one note gets its link, a row whose candidates are all gone demotes to an ordinary broken link. The table lives outside `SCHEMA_SQL`, so a vault that never has an ambiguous link never grows it. **Scoped tokens read this surface on their own sub-vault**: `candidate_count` is derived vault-wide, so it is re-resolved and narrowed to the visible candidates, and `has_ambiguous_links: false` lifts the SQL filter rather than leaking the collision by absence — otherwise the new surface would have been a standing, sweepable census of cross-scope naming collisions.
- **A scoped reader's broken-link answer no longer moves when an out-of-scope candidate is deleted (#712, refs #239, #707).** #707 narrowed the *ambiguous* surface to a scoped reader's sub-vault; the delete-time self-heal was still scope-blind, so `refreshAmbiguousLinks` demoting an ambiguous row into `unresolved_wikilinks` flipped a `work`-scoped reader's own note from `has_broken_links: false` to `true` — gaining a `broken_links` entry and an `/unresolved-wikilinks` row — purely because two notes it can never see were deleted. That is a timing oracle for exactly the cross-scope naming collision #707's `candidate_count` narrowing was added to hide. The stable sub-vault answer is *broken in both states* (`[[Dup]]` matches zero visible notes before and after), so resolution is deferred to read time for scoped readers, reusing #707's machinery rather than adding a second visibility model. `visibleResolutionCount` becomes the single place both scoped link surfaces decide what a reference means (≥2 visible → ambiguous, 0 → broken, 1 → resolves), honouring an already-resolved `WikilinkResolution` explicitly so a stale row's empty `candidates` array does not read as "0 visible". `getUnresolvedLinksForNotes` takes the same injected predicate and folds in ambiguous rows with no visible candidate; `sqlHasBrokenLinks` lifts **both** polarities out of SQL under scope — unlike the ambiguity filter, `true` is not a superset either, because a note can be broken in the sub-vault with no `unresolved_wikilinks` row at all — and `narrowByVisibleBrokenness` re-decides per note. `/unresolved-wikilinks` folds in the same rows so the third surface is stable too. Core stays scope-unaware and only invokes the injected predicate; **unscoped readers are byte-identical** — no predicate, no extra query, the persisted tables answer directly.
- **Write-time `ambiguous_link` warnings no longer leak a vault-wide `candidate_count` to scoped writers (#717, closes #716).** `create-note` / `update-note` echo per-note `warnings`, and an `ambiguous_link` entry carried a `candidate_count` derived vault-wide. Neither door narrowed it for a tag-scoped session — MCP's `scrubWriteResult` scrubbed `.tags`, `.links` and `.validation_status` but never `.warnings`, and REST had no scrub on any of its three write-response sites — so a `work`-scoped **writer** saving `[[Dup]]` learned from `candidate_count: 2` that a second `Dup` exists in a scope it cannot see: the oracle #707 closed on the read side, reachable through the write door. The warning is now re-decided per the same visibility closure via `narrowLinkWarningsForVisibility`, reusing core's `visibleResolutionCount` so read and write surfaces cannot drift — **≥2 visible → kept with the visible count; 1 visible → dropped (it resolves cleanly in the sub-vault); 0 visible → demoted to `unresolved_link`**, which is what an unscoped writer on the same sub-vault would see. Also folds the six duplicated warning literals into `ambiguousLinkWarning` / `unresolvedLinkWarning` so a scoped caller's warning text comes from the same template as the unscoped one. **Unscoped writers install no predicate and are unchanged.**
- **A rename cascades only over the brackets that actually resolved to the renamed note (#710, closes #708).** `cascadeRename` matched on the basename string alone. With `keep/Rename` and `move/Rename` both present, renaming `move/Rename` rewrote a source's *ambiguous* `see [[Rename]]` — a reference that pointed at nothing — and silently re-aimed it at the note that moved away. The cascade now follows resolution, not text: `planCascadeRename` takes its source set from the `links` rows whose `target_id` is the renamed note (plus the note itself, for self-referencing brackets, which deliberately never get a row) and re-resolves each bracket before the row moves. Bracket shape is preserved; a basename widens to a full path only when the new basename would no longer resolve back. Two incidental behaviour changes, both strictly better and pinned: `[[move/Rename.md]]` (the explicit-extension form) is now cascaded where the old regex left it dangling, and a `[[link]]` inside a code fence is no longer rewritten — it never resolved and never had a row, and the old content-wide regex edited it anyway. Ambiguous brackets are left to #707's sweep instead.
- **Note updates are transactional; hooks and cache invalidation fire post-commit (#711).** `updateNote` and its derived writes now run inside the Store transaction seam, and `invalidateConfigCachesForPath` + the `updated` hook dispatch are deferred through a new `afterCommit` seam until the entire surrounding transaction stack commits. A nested transaction merges its callbacks into its parent when its `SAVEPOINT` is released, so only the outermost successful commit runs them and a rollback discards the frame — a rolled-back batch now emits no hooks at all, where previously an inner closure could dispatch for a write that never landed. Outside a transaction the callback still runs immediately, so a single un-batched update is unchanged. Adds rollback coverage for a mid-update SQLite failure and for two updates inside an outer `transactionAsync` batch.
- **An out-of-scope query tag matches nothing on every door (#702, closes #675).** A tag-scoped token could NAME an out-of-scope tag in a query and read the answer off hit/miss — a note tagged `["mine","project-manhattan"]` is admitted via `mine`, so `?tag=project-manhattan` came back with a row and confirmed the guessed name. #568/#674 closed the disclosure; this closes the membership oracle it left, on REST (`?tag=` / `?exclude_tag=`), MCP `query-notes`, and live subscriptions. An out-of-scope tag now behaves exactly as a tag that does not exist — in filter position that is 200-empty, not a 400, because a rejection would make an out-of-scope tag look *different* from a nonexistent one and invert the rule. Implemented as an input rewrite so the response is the nonexistent-tag code path itself, byte-identical including the cursor envelope and the aggregate rollup. **Behaviour change, scoped tokens only:** a scoped token can no longer query by a shared tag outside its allowlist and get its own in-scope notes back. Unscoped tokens are unaffected.
- **`migrateToV28` versions the `unresolved_wikilinks` relationship-column heal (#703, vault#567 item 2).** The pre-#555 2-column rebuild used to run lazily on first touch; it now runs at `initSchema` so boot is deterministic. `SCHEMA_VERSION` 27 → 28. Gated the same way it always was — no table (the common case) is a no-op with no `CREATE`, an already-3-column table is a no-op, and only a pre-#555 2-column table is rebuilt, atomically. Pending forward-refs, not notes. Lazy table *creation* is unchanged.
- **MCP note mutations without an id are rejected, not crashed (#706, refs #705).** `update-note` and `delete-note` validate the identifier before resolution, so an omitted or blank id returns the established `missing_required_field` JSON-RPC error with an id-or-path hint instead of leaking a raw `TypeError`. The REST collection routes already answered 405 `method_not_allowed`, so no REST change was needed.
- **Bun-door description type guard, `merge-tags` `updated_at` bump, concurrent-PUT envelope pin (#701).** Three small follow-ups: `vault-info` (MCP) and `PATCH /api/vault` (REST) runtime-guard a non-string description into the existing `invalid_description` shape (`null` still clears), and `projectionToMarkdown` skips a non-string rather than throwing on initialize for an already-poisoned reconnect (vault#669 / cloud#87). `mergeTags` now bumps `updated_at` + `updated_at_ms` on notes whose tags actually changed, matching tags-only `update-note` so cursor/sync consumers see the retag; notes that never carried a merged-away source are untouched (vault#567 item 1). Plus the concurrent-PUT sibling of the serialized last-writer-wins test, asserting the safety envelope only (vault#662).
- **The filtered total's filter surface is pinned (#704, refs #626).** `queryNotes` and `aggregateNotes` share `buildFilterConditions`, so `?aggregate[op]=count` running over the same filter surface as the list held by construction — but nothing pinned it, and a filter handled outside the shared builder would have narrowed the list while the count kept reporting the unfiltered total, suite still green. Adds a table-driven pin over eight filters asserting `count == list length` AND `0 < n < TOTAL`, so a param both paths ignore cannot satisfy the equality vacuously. Also corrects `HTTP_API.md`: cloud's REST door now answers 400 `unsupported_param` for `aggregate[...]` rather than silently returning note rows, and cloud's MCP surface (core-driven, already serving grouped rollups) is documented. Docs + tests only.
- **`bun run test:core` works again (#709, refs #700).** The root script pointed at a Vitest CLI under `core/node_modules` that does not exist — core is a `bun:test` package. Repointed at the documented Bun core door (`bun test ./core/src/`). CI already invoked `bun test ./core/src/` directly and was never affected; `web/ui` keeps its separate Vitest suite.

Known follow-up: **#714** — resolved link *edges* are still computed scope-blind, so a scoped reader's `include_links` / `link_count` / `near` / `find-path` / backlinks can still move when an out-of-scope candidate is deleted. Not a confidentiality breach; a correctness and consistency gap that needs its own link-graph contract design.
...
isolation that stopped the suite writing real vaults.** Everything merged to
`next` after 0.7.6. Soaks on `@rc`; stable later is a suffix-drop only.

- **FTS search threads the full filter set on both doors (#656).**
  `exclude_tag`, date range, and the rest of the structured-query filters
  were silently dropped under `search=` — a query that looked filtered was
  not. They now apply through FTS the same way they apply without it.

- **`exclude_path_prefix` on REST and MCP (#658).** Drop notes whose path
  matches any prefix (e.g. `Log/` or `.parachute/`) without fetching the
  whole vault and filtering client-side.

- **Empty `include_metadata` stays `{}` (#657).** Projecting no metadata
...
- **Tag-scoped reads filter `.tags` to the in-scope subset (#674, closes
  #568).** A scoped token used to see a note's full tag list, including
  tags outside the grant. REST and MCP now scrub to the in-scope set,
  including on update echoes.

- **Wikilinks: ID-valued forward-refs heal (#666); unicode-fold titles
  (#668).** A `[[id]]` written before the target existed resolves when it
  arrives. Title/ext matching folds unicode so `café` and `café` hit the
  same note.

- **Transcription: python-provider `--help` probe (#667); download
  checksum + repair (#673).** A python binary that exists but is not the
...
- **Add-pack absorbs a lost create race (#672)** and sets `busy_timeout`.

- **Ticket download Content-Type comes from the extension (#660).**

- **Suite isolation is unconditional (#678).** The preload always replaces
