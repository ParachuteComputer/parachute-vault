...
**A returned note's `.tags` is filtered to the in-scope subset** (vault#568, 2026-08-22). A scoped token is admitted to a note when ANY of its tags is in scope — so a note tagged `["mine","project-manhattan"]` is readable by a `mine`-scoped token. The response's `.tags` array is narrowed to exactly the tags that token can see (`["mine"]`), using the same per-tag rule that admitted the note. Otherwise being allowed to read a co-tagged note would disclose the NAMES of every other tag on it — the same out-of-scope-tag-name disclosure class as the `validation_status` scrub and the `group_by: "tag"` group-name scrub. Applies identically on **both doors** (REST `GET /api/notes[…]`, MCP `query-notes`) and on the **live-subscription transport** (WS snapshot frames + `upsert` events), and to **write responses** (`POST`/`PATCH /api/notes`, `create-note`, `update-note`) because those echo the stored note — otherwise a no-op update would be a one-call bypass. Hydrated `include_links` endpoint summaries get the same treatment on their own `tags`. **Unscoped tokens are unaffected** — they still receive the full tag set everywhere.

Two consequences worth stating explicitly:

- **The array is never empty for a note the caller can see.** A note whose tags are ALL out of scope fails the auth check and is already invisible (404 / silently filtered), so it never reaches the scrub. The fix introduces no new "note with no tags" shape — consistent with §Semantics' "out-of-scope reads return 404, not 403": the scope boundary is invisible, not redacted in place.
- **Storage is untouched, and no read-modify-write can clobber a hidden tag.** The scrub shapes responses only. Tag mutation is additive-union on every path (`tags.add`/`tags.remove` on update; `if_exists: update|replace` unions `existing ∪ incoming`) — there is no wholesale tag-replace API — so a client echoing a scrubbed `.tags` back cannot silently delete the out-of-scope tags it can't see.

**An out-of-scope tag NAMED IN A QUERY matches nothing** (vault#675, 2026-09-02). Filtering `.tags` (above) stopped scoped reads from *disclosing* an out-of-scope co-tag's name; a scoped caller could still name one itself — `GET /api/notes?tag=project-manhattan`, `query-notes { tag }`, `GET /api/subscribe?tag=` — and read the answer off hit/miss, because a co-tagged note is admitted via its OTHER tag. No name leaked; membership did. The rule now: **an out-of-scope tag behaves exactly as a tag that does not exist**, the same equivalence this contract already runs on elsewhere (an out-of-scope note 404s like a missing one; `list-tags { tag }` / `GET /api/tags?tag=` answer `tag_not_found` for an out-of-scope name "whether the tag exists or not"). In the *filter* position a nonexistent tag is not an error — it is a filter that matches nothing — so this is **not** a 400/404: the query still runs and still returns 200, it just cannot match. That choice follows §Semantics' partial-graph stance (the boundary is invisible, not marked "you can't see this") rather than contradicting it with a loud rejection.

Composition falls out of that single rule, with no special cases: `tag=X` returns `[]`; `tag=mine&tag=X&tag_match=all` returns `[]` (dropping `X` instead would have *widened* the answer); `tag_match=any` gets a union contribution of nothing; `exclude_tag=X` excludes nothing. Enforced on all three doors — REST `?tag=` / `?exclude_tag=` (every branch: structured, `search`, `semantic`, `aggregate`, `near`, cursor, `format=graph`), MCP `query-notes`, and live subscriptions (snapshot + events, scoped once so matcher/snapshot parity holds by construction). **Unscoped tokens are unaffected.**
