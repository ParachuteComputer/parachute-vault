...
- ~~**Vault as OAuth issuer (Phase 0/1):** vault mints `pvt_*` opaque tokens. `oauth.ts` checks `authCode.vault_name !== vaultName` before issuing — vault-issued tokens are vault-pinned by construction.~~ **(Retired 2026-05-25 — workstream E. Vault is OAuth resource-server-only; hub is the issuer. The CLI-minted `pvt_*` non-OAuth path was then fully dropped at 0.6.0 — vault#282 Stage 2 — so vault no longer mints or accepts `pvt_*` at all; minting is the hub's job via `parachute auth mint-token`.)**
